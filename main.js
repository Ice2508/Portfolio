(() => {
   ...
   const videoSrc = [{
         path: 'video/work1.mp4',
         text: [
            'Разработан одностраничник для ниши «Ремонт балконов под ключ» с акцентом на высокую скорость загрузки и удобство пользователя.',
            'Интерактивный 5-ступенчатый калькулятор: Реализован сложный алгоритм расчета стоимости, учитывающий тип балкона, точные размеры, материалы отделки (стены/пол/потолок) и дополнительные опции (электрика/мебель).',
            'Настроена система сбора заявок, которая передает владельцу не просто контактные данные, а полную спецификацию выбранных в калькуляторе параметров для подготовки точного КП.',
            'Полная адаптивность, использование WebP для скорости, внедрение SEO-микроразметки (JSON-LD) и настроенная аналитика (Yandex.Metrika).'
         ]
      },
      {
         path: 'video/work2.mp4',
         text: [
            'Многофункциональная платформа для кинотеатра: Полноценное одностраничное приложение (SPA) с полным циклом: от выбора фильма до покупки билета.',
            'Реализована удобная навигация с календарем сеансов и визуальной схемой зала для выбора конкретных мест в реальном времени.',
            'Полноценная панель администратора: Защищенный интерфейс, позволяющий владельцу полностью управлять контентом: создавать залы, добавлять фильмы и гибко настраивать сетку сеансов.',
            'Автоматическая генерация данных для билета (включая QR-коды) и сохранение статуса бронирования, что исключает ошибки при оформлении.'
         ]
      },
      {
         path: 'video/work3.mp4',
         text: [
            'Мобильное приложение «Кулинарная книга» (APK): Кроссплатформенное решение для создания и хранения рецептов с упором на высокую производительность и удобство мобильного интерфейса.',
            'Реализовано полноценное использование приложения без доступа к интернету благодаря локальному хранилищу данных, что гарантирует мгновенный доступ к рецептам в любых условиях.',
            'Разработана система резервного копирования, позволяющая при наличии сети отправлять локальные данные на сервер для надежного хранения.',
            'Внедрена система фильтрации по категориям и ингредиентам, а также реализована динамическая смена тем оформления (Dark/Light mode) для комфортного использования.'
         ]
      }
   ];
   ...
   function popupMessOk(popupSubmit) {
      popupSubmit.innerHTML = `
        <button class="popup-submit__close" type="button">×</button>
        <img class="popup-submit__icon" src="img/popup-submit-icon.webp">
        ...
        <button class="btn-gold close" data-popup-close>закрыть</button>
    `;
   }
   ...
   function popupMessError(popupSubmit) {
      popupSubmit.innerHTML = `
        <button class="popup-submit__close" type="button">×</button>
        <img class="popup-submit__icon" src="img/popup-submit-error-icon.webp">
        <div class="popup-submit__text-wrap">
           <h3 class="popup-submit__title">Ошибка отправки</h3>
           <p class="popup-submit__subtitle">Повторите попытку чуть позже</p>
        </div>   
        <button class="btn-gold back" type="button" data-popup-close>назад</button>
    `;
   }
   ...
   function popupInterfaceReset(popupSubmit) {
      popupSubmit.innerHTML = `
        <button class="popup-submit__close" type="button">×</button>
          ...
          <button class="btn-gold popup-submit__btn" type="submit">отправить</button>
      `
   }
